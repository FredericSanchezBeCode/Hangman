...
let dictionnaire=['ordinateur','javascript','smartphone','meteorologie','frederic'];
let mot = dictionnaire[array_rand(dictionnaire, 1)];
let  arr_word_to_find = str_split(mot);
let arr_word_found = [] ;
...
if(!isset(arr_word_to_find)){
   
    for(i = 0 ; i < strlen(mot); i++){
    
        array_push(arr_word_found,"_");    
    }
    let lettresJouees = [];
    let pas_dedans= false;
	let nb_essais_restants=8;
    
}
if(isset(lettreentree)){
    pas_dedans= true;
	
    array_push(lettresJouees, lettreentree);
    if(in_array(lettreentree, arr_word_to_find)){
        foreach(arr_word_to_find as i => v){
            if(v == lettreentree){
                arr_word_found[i]=v;
                pas_dedans= false;
            }
...
for(i = 0 ; i < count(arr_word_found); i++){
    
    alert( arr_word_found[i]. ' ';
}
...
if(arr_word_to_find==arr_word_found){ 
    alert( '<br/>'.'<h1>victoire !</h1>'.'<br/>');
    alert( '<img src="images/champions.png" class="img">');    
}
else{
	if(nb_essais_restants==0){
		alert( '<br/>'.'<h1>T es mauvais !</h1>'.'<br/>');
		alert( '<img src="images/loser.png" class="img">)';
	}
}



if((arr_word_to_find!=arr_word_found) && (nb_essais_restants>0)){
    foreach(range('a','z') as lettre){
    alert( '<button type="submit"');
	if(in_array(lettre, lettresJouees)){
		alert( (' disabled ');
	}
	alert( 'value='.lettre.' name="lettreentree">'.lettre.'</button> ');
    } 
}
/*	var_dump(lettresJouees);  */
